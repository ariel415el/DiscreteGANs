...
argument type using `--z_prior const=N` to make it have a discrete latent space of size `N`

For example, here is the Python command we used to train discreteWGAN with an FC discriminator for Figure 5 in the paper
```
...
We implemented the direct optimization under the same script. The first thing is to change the loss function with the argument
...
For both WGAN and direct Patch SWD you have a look at all the possible arguments for the **train.py** script [here](utils/train_utils.py).
...
All experiments on FFHQ were done with --center crop 90

## Figure 2: SOTA discrete GAN 
We trained this model on 128x128 images but, to crop more focused images around the face not upscale from 90 to 128 
